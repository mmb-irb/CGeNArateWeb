...
var TableDatatablesEditable = function () {
    ...
    var handleTable = function () {
        ...
        function editRow(oTable, nRow) {
            ...
            jqTds[7].innerHTML = '<input type="number" class="form-control input-xsmall input-sm" min="1" max="' + diskLimit + '" value="' + aData[7] + '">';
            jqTds[8].innerHTML = '<div class="btn-group"><button class="btn btn-xs yellow-mint dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Actions <i class="fa fa-angle-down"></i></button><ul class="dropdown-menu pull-right" role="menu"><li><a class="edit" href="javascript:;"><i class="fa fa-save"></i> Save user</a></li><li><a class="cancel" href="javascript:;"><i class="fa fa-times-circle"></i> Cancel edition</a></li></ul></div>';
        }
        ...
        function saveRow(oTable, nRow) {
					...
					if((jqInputs[0].value != '')
						...
						if (jqSelects[1].value == 0) {
							oTable.fnUpdate('<div class="btn-group"><button disabled class="btn btn-xs green dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false" style="opacity:1;"> Admin <i class="fa fa-user-times"></i></button></div>', nRow, 5, false);
						}else {
			...
			'<div style="display:none;">*' + jqSelects[1].value + '*</div>', nRow, 5, false);
						...
						if (jqSelects[1].value == 0) {
							...
							oTable.fnUpdate('<div class="btn-group"><button class="btn btn-xs yellow-mint dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Actions <i class="fa fa-angle-down"></i></button><ul class="dropdown-menu pull-right" role="menu"><li><a class="edit" href="javascript:;"><i class="fa fa-pencil"></i> Edit user</a></li>  <li><a class="enable"  href="javascript:;"><i class="fa fa-ban"></i> Disable user</a></li></ul></div>', nRow, 8, false);
						}
							oTable.fnDraw();
							return true;
					}else{
        ...
        function saveNewRow(oTable, nRow) {
            ...
            && (jqSelects[1].value != '')) {
              ...
              else oTable.fnUpdate('<div class="btn-group"><button class="btn btn-xs yellow-mint dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Actions <i class="fa fa-angle-down"></i></button><ul class="dropdown-menu pull-right" role="menu"><li><a class="edit" href="javascript:;"><i class="fa fa-pencil"></i> Edit user</a></li>  <li><a class="enable"  href="javascript:;"><i class="fa fa-ban"></i> Disable user</a></li></ul></div>', nRow, 8, false);
              oTable.fnDraw();
              return true;
            }else{
              $('#myModal2').modal('show');
        ...
        table.on('click', '.edit', function (e) {
            ...
            if (nEditing !== null && nEditing != nRow) {
                ...
                if(saveRow(oTable, nEditing)) {
                  ...
                  var strData = 'id=' +  $('input.idusr', nRow).val() + '&name=' + jqInputs[1].value + '&surname=' + jqInputs[0].value + '&institution=' + jqInputs[2].value + '&diskQuota=' + jqInputs[3].value + '&country=' + jqSelects[0].value + '&type=' + jqSelects[1].value;
                ...
                if(saveNewRow(oTable, nEditing)) {
                  ...
                  console.log(strData);
									...
									$.ajax({
												type: "POST",
												url: "/applib/newUserFromAdmin.php",
												data: strData, 
		...
		var nRow = $(this).parents('tr')[0];
			...
			if(this.innerHTML.indexOf("Enable user") != -1 ) {
				...
				$.ajax({
					type: "GET",
					url: baseURL + "admin/change/status/" + $('input.idusr', nRow).val() + "/1",
					success: function(d) {
						data = $.parseJSON(d);
				...
				$.ajax({
           			type: "GET",
           			url: baseURL + "admin/change/status/" + $('input.idusr', nRow).val() + "/0",
           			success: function(d) {
						data = $.parseJSON(d);
		...
		table.on('click', '.role-usr', function (e) {
			...
			$.ajax({
           		type: "GET",
           		url: baseURL + "admin/change/type/" + $('input.idusr', nRow).val() + "/" + idRole + "/" + oldRole,
           		success: function(d) {
...
jQuery(document).ready(function() {
		...
		$('#modalDelete').find('.modal-footer .btn-modal-del').on('click', function(){
			...
			$.ajax({
				...
				type: "GET",
				url: baseURL + "admin/delete/user/" + delUsrID,
				success: function(data) {
